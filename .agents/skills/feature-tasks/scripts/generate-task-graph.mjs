...
function parseRequiredString(value, label, errors2) {
  const text = normalizeString(value);
  if (!text) {
    errors2.push(`${label} is required`);
    return "";
  }
  return text;
}
...
function parseOptionalStringList(value, label, errors2) {
  if (value === undefined || value === null)
    return [];
  if (!Array.isArray(value)) {
    errors2.push(`${label} must be an array of strings`);
    return [];
  }
  const items = [];
  for (let i = 0;i < value.length; i += 1) {
    ...
    if (!item) {
      errors2.push(`${label}[${i}] must be a non-empty string`);
      continue;
    }
    items.push(item);
  }
...
function parseRequiredStringList(value, label, errors2, { minItems = 1 } = {}) {
  if (value === undefined || value === null) {
    errors2.push(`${label} is required`);
    return [];
  }
  if (!Array.isArray(value)) {
    errors2.push(`${label} must be an array of non-empty strings`);
    return [];
  }
  const items = [];
  for (let i = 0;i < value.length; i += 1) {
  ...
  if (items.length < minItems) {
    errors2.push(`${label} must contain at least ${minItems} item(s)`);
  }
  return items;
}
function parseIdentifierList(value, label, errors2, { required = true } = {}) {
  if (value === undefined || value === null) {
    if (required) {
      errors2.push(`${label} is required`);
    }
  ...
  if (!Array.isArray(value)) {
    errors2.push(`${label} must be an array of task IDs`);
    return [];
  }
  const deduped = [];
  const seen = new Set;
  for (let i = 0;i < value.length; i += 1) {
    ...
    if (!taskId) {
      errors2.push(`${label}[${i}] must be a non-empty task ID`);
      continue;
    }
    if (seen.has(taskId))
      continue;
...
function assertUniqueIds(items, key, label, errors2) {
  const seen = new Set;
  ...
  if (duplicates.size > 0) {
    errors2.push(`${label} contains duplicate IDs: ${Array.from(duplicates).sort().join(", ")}`);
  }
}
function parseTasksYaml(content) {
  let document;
  try {
    document = $parse(content.replace(/^\uFEFF/, ""));
  } catch (error) {
  ...
  if (!Array.isArray(rawTasks)) {
    errors2.push("tasks is required and must be an array");
  } else if (rawTasks.length === 0) {
    errors2.push("tasks must contain at least one task");
  }
  const tasks = Array.isArray(rawTasks) ? rawTasks.map((task, index) => {
    const fieldErrors = [];
    if (typeof task !== "object" || task === null || Array.isArray(task)) {
      errors2.push(`tasks[${index}] must be an object`);
      return {
        id: `tasks[${index}]`,
        phase: "",
        priority: "",
    ...
    if (priority && !PRIORITIES.has(priority)) {
      fieldErrors.push(`tasks[${index}].priority must be one of: ${Array.from(PRIORITIES).join(", ")}`);
    }
    ...
    const deliverables = parseRequiredStringList(task.deliverables, `tasks[${index}].deliverables`, fieldErrors, { minItems: 1 });
    ...
    if (ownerType && !OWNER_TYPES.has(ownerType)) {
      fieldErrors.push(`tasks[${index}].owner_type must be one of: ${Array.from(OWNER_TYPES).join(", ")}`);
    }
    ...
    if (fieldErrors.length > 0) {
      ...
      errors2.push(`[${taskKey}] ${fieldErrors.join("; ")}`);
    }
    return {
      id,
      phase,
  ...
  const parseGroup = (key, label) => {
    ...
    if (!Array.isArray(raw)) {
      errors2.push(`${key} must be an array when provided`);
      return [];
    }
    return raw.map((entry, index) => {
      const fieldErrors = [];
      if (typeof entry !== "object" || entry === null || Array.isArray(entry)) {
        errors2.push(`${key}[${index}] must be an object`);
      ...
      if (fieldErrors.length > 0) {
        errors2.push(`[${id || `${label}[${index}]`}] ${fieldErrors.join("; ")}`);
      }
  ...
  assertUniqueIds(tasks, "id", "tasks", errors2);
  assertUniqueIds(criticalPaths, "id", "critical_paths", errors2);
  assertUniqueIds(parallelWindows, "id", "parallel_windows", errors2);
  if (errors2.length > 0) {
...
function detectDependencyCycles(tasks) {
  ...
  function dfs(taskId, stack) {
    ...
    if (nodeState === 1) {
      ...
      errors2.push(`Dependency cycle detected: ${cyclePath.join(" -> ")}`);
      return;
    }
    if (nodeState === 2)
      return;
...
function validateGraphParity(parsed) {
  ...
  for (const task of parsed.tasks) {
    ...
    if (taskErrors.length > 0) {
      errors2.push(`[${task.id}] ${taskErrors.join("; ")}`);
    }
  }
  for (const criticalPath of parsed.criticalPaths) {
    for (const taskId of criticalPath.tasks) {
      if (!taskIds.has(taskId)) {
        errors2.push(`[critical_paths.${criticalPath.id}] references missing task ${taskId}`);
      }
    }
  }
  for (const parallelWindow of parsed.parallelWindows) {
    for (const taskId of parallelWindow.tasks) {
      if (!taskIds.has(taskId)) {
        errors2.push(`[parallel_windows.${parallelWindow.id}] references missing task ${taskId}`);
      }
    }
  }
  errors2.push(...detectDependencyCycles(parsed.tasks));
  if (errors2.length > 0) {
...
`);
    console.info(`Generated ${path.relative(cwd, outputPath)} for planning/${slug} (${graph.nodes.length} nodes, ${graph.edges.length} edges)`);
  } catch (error) {
