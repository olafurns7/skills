...
var VALID_STATES = new Set(["todo", "in_progress", "done", "blocked"]);
var VALID_RESULTS = new Set(["done", "blocked", "failed"]);
var TASK_ARRAY_KINDS = ["blockers", "files_changed", "tests_run", "next_unblocked_tasks"];
var STATUS_CONTEXT = new WeakMap;
...
function toPlanningSlug(rawValue) {
  ...
  if (!slug)
    fail(`Unable to derive valid slug from "${rawValue}"`);
  return slug;
}
...
function parseIdentifierArray(value, label) {
  if (value === undefined || value === null)
    return [];
  if (!Array.isArray(value))
    fail(`${label} must be an array of task IDs`);
...
function readTasksFromYaml(tasksPath) {
  ...
  let document;
  try {
    document = $parse(fs.readFileSync(tasksPath, "utf8").replace(/^\uFEFF/, ""));
  } catch (error) {
  ...
  for (let i = 0;i < rawTasks.length; i += 1) {
    ...
    if (tasksById[id])
      fail(`tasks.yaml contains duplicate task id: ${id}`);
    ...
    const context = Array.isArray(task.context) ? task.context.map((item) => normalizeString(item)).filter(Boolean) : [];
    tasksById[id] = {
      id,
...
function readTasksFromGraph(graphPath) {
  ...
  for (let i = 0;i < graph.nodes.length; i += 1) {
    ...
    if (tasksById[id])
      fail(`task.graph.json contains duplicate node id: ${id}`);
    tasksById[id] = {
      id,
      ...
      context: Array.isArray(node.context) ? node.context.map((item) => normalizeString(item)).filter(Boolean) : []
    };
    taskOrder.push(id);
  }
...
function normalizeTaskStatusRecord(value) {
  ...
  normalized.blockers = Array.isArray(record.blockers) ? record.blockers.map((item) => normalizeString(item)).filter(Boolean) : [];
  normalized.files_changed = Array.isArray(record.files_changed) ? record.files_changed.map((item) => normalizeString(item)).filter(Boolean) : [];
  normalized.tests_run = Array.isArray(record.tests_run) ? record.tests_run.map((item) => normalizeString(item)).filter(Boolean) : [];
  normalized.result_summary = normalizeString(record.result_summary);
  normalized.next_unblocked_tasks = Array.isArray(record.next_unblocked_tasks) ? record.next_unblocked_tasks.map((item) => normalizeString(item)).filter(Boolean) : [];
  return normalized;
}
function openStateDb(paths) {
  ...
  `);
  db.query("INSERT INTO meta(key, value) VALUES ('schema_version', ?1) ON CONFLICT(key) DO UPDATE SET value = excluded.value").run(String(DB_SCHEMA_VERSION));
  return db;
}
function readTaskArray(db, taskId, kind) {
  const rows = db.query("SELECT value FROM task_arrays WHERE task_id = ?1 AND kind = ?2 ORDER BY idx ASC").all(taskId, kind);
  return rows.map((row) => normalizeString(row.value)).filter(Boolean);
}
function readTaskRecordFromDb(db, taskId) {
  const row = db.query("SELECT state, owner, attempts, started_at, finished_at, result_summary FROM tasks WHERE task_id = ?1").get(taskId);
  if (!row) {
...
function upsertTaskRecord(db, taskId, record) {
  ...
  db.query(`INSERT INTO tasks(task_id, state, owner, attempts, started_at, finished_at, result_summary)
       ...
       ON CONFLICT(task_id) DO UPDATE SET
         ...
         result_summary = excluded.result_summary`).run(taskId, safeRecord.state, safeRecord.owner, safeRecord.attempts, safeRecord.started_at, safeRecord.finished_at, safeRecord.result_summary);
  db.query("DELETE FROM task_arrays WHERE task_id = ?1").run(taskId);
  const insertArray = db.query("INSERT INTO task_arrays(task_id, kind, idx, value) VALUES (?1, ?2, ?3, ?4)");
...
function bootstrapDbForPlan(db, paths, plan, warnings) {
  const existingProject = normalizeString(db.query("SELECT value FROM meta WHERE key = 'project'").get()?.value);
  if (existingProject && existingProject !== plan.project) {
    warnings.push(`Status project "${existingProject}" does not match plan project "${plan.project}". Using plan project.`);
  }
  db.query("INSERT INTO meta(key, value) VALUES ('project', ?1) ON CONFLICT(key) DO UPDATE SET value = excluded.value").run(plan.project);
  db.query("INSERT INTO meta(key, value) VALUES ('updated_at', ?1) ON CONFLICT(key) DO UPDATE SET value = excluded.value").run(new Date().toISOString());
  ...
  if (existingTaskCount === 0 && fs.existsSync(paths.statusPath)) {
    ...
    if (snapshot.project && snapshot.project !== plan.project) {
      warnings.push(`Imported status project "${snapshot.project}" does not match plan project "${plan.project}". Using plan project.`);
    }
    for (const taskId of plan.taskOrder) {
  ...
  const insertDefaultTask = db.query(`INSERT INTO tasks(task_id, state, owner, attempts, started_at, finished_at, result_summary)
  ...
  const insertPlanTask = db.query(`INSERT INTO plan_tasks(task_id, title, blocked_by_json, acceptance_json, deliverables_json, context_json)
     VALUES (?1, ?2, ?3, ?4, ?5, ?6)`);
  for (const taskId of plan.taskOrder) {
    const task = plan.tasksById[taskId];
    insertPlanTask.run(taskId, task.title, JSON.stringify(task.blocked_by), JSON.stringify(task.acceptance), JSON.stringify(task.deliverables), JSON.stringify(task.context));
  }
}
...
function canUnblockFromDependencies(blockers, tasks, knownTaskIds) {
  if (!Array.isArray(blockers) || blockers.length === 0)
    return false;
  if (blockers.some((blocker) => !knownTaskIds.has(blocker)))
    return false;
...
function writeStatus(paths, status) {
  ...
  try {
    ...
    db.query("INSERT INTO meta(key, value) VALUES ('project', ?1) ON CONFLICT(key) DO UPDATE SET value = excluded.value").run(status.project);
    db.query("INSERT INTO meta(key, value) VALUES ('updated_at', ?1) ON CONFLICT(key) DO UPDATE SET value = excluded.value").run(status.updated_at);
    db.exec("COMMIT");
    const snapshot = {
...
function getReadyTaskIds(plan, status) {
  return plan.taskOrder.filter((taskId) => {
    ...
    if (!taskStatus || taskStatus.state !== "todo")
      return false;
    return taskIsUnblocked(plan.tasksById[taskId], status);
  });
}
function headingSlug(text) {
  return normalizeString(text).toLowerCase().replace(/[`~!@#$%^&*()+=[\]{}|\\:;"'<>,.?/]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-").replace(/^-+|-+$/g, "");
}
...
function extractMarkdownSections(content, targetAnchors) {
  ...
  for (let i = 0;i < headings.length; i += 1) {
    ...
    if (!targetAnchors.has(current.slug))
      continue;
    if (usedStarts.has(current.line))
      continue;
...
function formatDependencyResultsForPrompt(dependencyResults) {
  ...
  return dependencyResults.map((item) => {
...
function commandInit(paths, jsonMode) {
  ...
  process.stdout.write(`Resumed: ${status.summary.done} done, ${status.summary.todo} todo, ${status.summary.blocked} blocked
`);
...
function resolveTaskForPrompt(plan, status, explicitTaskId) {
  if (explicitTaskId) {
    if (!plan.tasksById[explicitTaskId])
      fail(`Unknown task: ${explicitTaskId}`);
    return explicitTaskId;
  }
  const ready = getReadyTaskIds(plan, status);
  if (ready.length === 0)
    fail("No ready tasks available");
  return ready[0];
}
...
function commandStart(paths, taskId, owner, jsonMode) {
  ...
  process.stdout.write(`Started ${taskId} (owner=${record.owner}, attempts=${record.attempts}). In-progress: ${status.summary.in_progress}
`);
}
function commandDispatch(paths, taskId, owner, jsonMode) {
  ...
  process.stdout.write(`Dispatched ${resolvedTaskId} (owner=${record.owner}, attempts=${record.attempts}). In-progress: ${status.summary.in_progress}
`);
}
function commandComplete(paths, taskId, flags, jsonMode) {
  ...
  process.stdout.write(`Completed ${taskId} as ${record.state}. Ready tasks (${ready.length}): ${ready.join(", ") || "(none)"}
`);
}
function commandStatus(paths, jsonMode) {
  ...
  process.stdout.write(`Summary: todo=${status.summary.todo} in_progress=${status.summary.in_progress} done=${status.summary.done} blocked=${status.summary.blocked}
`);
...
function main() {
  ...
  if (command === "start") {
    ...
    if (!normalizedTaskId)
      fail("Usage: taskctl start [title-slug] <task-id> --owner <owner-name>");
  ...
  if (command === "complete") {
    ...
    const normalizedTaskId = normalizeString(taskId);
    if (!normalizedTaskId) {
      fail("Usage: taskctl complete [title-slug] <task-id> --result <done|blocked|failed> --summary <text>");
    }
    const paths = resolvePlanningPaths(cwd, slugArg);
